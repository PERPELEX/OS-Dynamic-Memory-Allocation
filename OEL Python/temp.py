...
class MemoryAllocationApp:
    ...
    def update_canvas(self):
        ...
        for i, block in enumerate(self.memory_manager.blocks):
            color = "#ff5959" if block.is_occupied else "springgreen2"
            self.canvas.create_rectangle(50, y, 550, y + 30, fill=color, outline="black")
            text = f"Block {i}: Used {sum(block.occupied_fragments)}KB, Free {block.free_size()}KB"
